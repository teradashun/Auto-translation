...
def translate_to_latex(outputs_dir: Path) -> None:
    ...
    1. 言語: 出力は必ず日本語で行うこと。技術用語や固有名詞を除き、英語の文章を出力しないこと。
    2. フォーマット: 標準的なLaTeX構文を使用すること。
    ...
    6. 出力: 出力は純粋なLaTeXコードのみ。Markdownのフォーマット（latex ...  など）は不要。
    7. プリアンブルなし: \documentclass, \usepackage, \begin{document},
